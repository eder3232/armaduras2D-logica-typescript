...
export class Armor2D {
  ...
  generateLocals() {
    for (const [key, value] of this.edges) {
      ...
      const transform = [[cx, 0], [cy, 0], [0, cx], [0, cy]];

      const transformTranspose = transpose(transform)
        .valueOf() as number[][];
      ...
      const complete_LocalCoordinates = multiply(
        value.ea_l,
        base,
      ) as number[][];
      ...
      this.localArrays.set(key, {
        transform: transform,
        transformTransposed: transformTranspose,
      ...
      tablePrinter(withoutEA_GlobalCoordinates, 2);
    }
  ...
  assembler(e: ILocalArrays) {
    ...
    for (let i = 0; i < lengthTableDOF; i++) {
      ...
      for (let j = 0; j < lengthTableDOF; j++) {
        ...
        this.k.global[row!][column!] += e.local_globalCoordinates[i][j];
      }
    }
  }
    ...
    if (this.settings.isRestrictedAbove) {
      for (const [key, value] of this.dataArray) {
        if (value["isRestricted"]) {
          ...
          this.f
            .global[this.utils.dofPointerInDataArray.get(value.dofInternal)!][
            0
          ] = `F-${key}`;
        } else {
          this.f
