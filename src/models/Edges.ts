...
export interface IEdge {
  ...
  DOF: IDOF
  coordinates: {
    xi: number, xj: number, zi: number, zj: number
  }
}
...
export class Edges {
  ...
  add({
    id, from, to, area, elasticity
  }: IInputEdge) {
    // Validaciones 
    // Verificar que el vertice no existe
    if (this.data.has(id)) {
      throw new Error(`La barra ${id} ya existe. Estos deben tener un nombre único.`)
    }
    // Verificar que el from y el to existen:
    if (this.vertices.has(from) === false) {
      throw new Error(`El vértice ${from}, asignado a la barra ${id} no existe.`)
    }
    ...
    const lengthEdge = Math.sqrt(Math.pow(toX - fromX, 2) + Math.pow(toZ - fromZ, 2))
    ...
    this.utils.axisDOF.map((a) => DOF.table.push({
    ...
    this.data.set(id, {
      ...
      DOF,
      coordinates: {
        xi: fromX, xj: toX, zi: fromZ, zj: toZ
      }
    })
  }
  ...
  getData(): IEdgesGetData {
    return {
      edges: this.data
      , vertices: this.vertices, utils: this.utils
    }
  }
}